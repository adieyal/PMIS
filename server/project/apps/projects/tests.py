...
class TestProject(TestCase):
    ...
    def test_actual_expenditure(self):
        date0 = datetime(2012, 6, 1)
        date1 = datetime(2013, 6, 1)
        date2 = datetime(2013, 7, 1)
        models.Project.objects.all().delete()
        project = factories.ProjectFactory()
        self.assertEqual(project.fy(date1).actual_expenditure, 0)

        factories.MonthlySubmissionFactory(project=project, actual_expenditure=100, date=date0)
        ...
        self.assertEqual(project.fy(date1).actual_expenditure, 100)
        self.assertEqual(project.fy(date2).actual_expenditure, 300)
        self.assertEqual(project.all(date2).actual_expenditure, 400)

    def test_actual_expenditure_with_previous_expenses(self):
        date1 = datetime(2012, 6, 1)
        date1 = datetime(2013, 6, 1)
        date2 = datetime(2013, 7, 1)
        models.Project.objects.all().delete()
        project = factories.ProjectFactory()

        factories.MonthlySubmissionFactory(project=project, actual_expenditure=50, date=date1)
        factories.MonthlySubmissionFactory(project=project, actual_expenditure=100, date=date1)
        factories.MonthlySubmissionFactory(project=project, actual_expenditure=200, date=date2)
        factories.ProjectFinancial(project=project, previous_expenses=500)

        self.assertEqual(project.all(date2).actual_expenditure, 850)
    ...
    def test_planned_expenditure(self):
        ...
        models.Project.objects.all().delete()
        project = factories.ProjectFactory()
        self.assertEqual(project.fy(date1).planned_expenditure, 0)

        factories.PlanningFactory(project=project, planned_expenses=50, date=date0)
        ...
        self.assertEqual(project.fy(date1).planned_expenditure, 100)
        self.assertEqual(project.fy(date2).planned_expenditure, 300)
    ...
    def test_total_actual_expenditure(self):
        ...
        self.assertEqual(models.Project.objects.fy.actual_expenditure(date1), 0)
        for i in range(5):
            project = factories.ProjectFactory()
            factories.MonthlySubmissionFactory(project=project, actual_expenditure=100, date=date1)
            factories.MonthlySubmissionFactory(project=project, actual_expenditure=100, date=date1)
        ...
        projects = models.Project.objects.fy.all()
        total = sum([p.fy(date1).actual_expenditure for p in projects])
        self.assertEqual(projects.fy.actual_expenditure(date1), total)

        # Should include both may and june expenditure
        total = sum([p.fy(date2).actual_expenditure for p in projects])
        self.assertEqual(projects.fy.actual_expenditure(date2), total)

        # Check that subsets are catered for - i.e. not just .all()
        district = projects[0].municipality.district
        projects = models.Project.objects.fy.district(district)
        total = sum([p.fy(date1).actual_expenditure for p in projects])
        self.assertEqual(projects.fy.actual_expenditure(date1), total)
        total = sum([p.fy(date2).actual_expenditure for p in projects])
        self.assertEqual(projects.fy.actual_expenditure(date2), total)
    ...
    def test_previous_expenditure(self):
        ...
        projects = models.Project.objects.all()
        total = sum([p.all(date2).actual_expenditure for p in projects])
        self.assertEqual(projects.alltime.actual_expenditure(date2), total)
    ...
    def test_total_planned_expenditure(self):
        ...
        # Should include only may expenditure
        projects = models.Project.objects.all()
        total = sum([p.fy(date1).planned_expenditure for p in projects])
        self.assertEqual(projects.total_planned_expenditure(date1), total)

        # Should include both may and june expenditure
        total = sum([p.fy(date2).planned_expenditure for p in projects])
    ...
    def test_percentage_actual_expenditure(self):
        ...
        models.Project.objects.all().delete()
        project = factories.ProjectFactory()
        self.assertEqual(models.Project.objects.fy.percentage_actual_expenditure(self.date), 0)
